...
//! by Wen Xia, et al., in 2016. It incorporates a simplified hash judgement
//! using the fast Gear hash, sub-minimum chunk cut-point skipping, and
//! normalized chunking to produce chunks of a more consistent length.
//!
...
///
/// The FastCDC chunker implementation. Use `new` to construct an instance, and
/// then iterate over the `Chunk`s via the `Iterator` trait.
///
...
/// let chunker = v2016::FastCDC::new(&contents, 8192, 16384, 65535);
...
impl<'a> FastCDC<'a> {
    ...
    pub fn new(source: &'a [u8], min_size: u32, avg_size: u32, max_size: u32) -> Self {
        ...
        // default to level 1 chunk size normalization
    ...
    pub fn cut(&self, start: u64, mut remaining: u64) -> (u64, u64) {
        ...
        let mut center = self.min_size + self.avg_size;
        if remaining > self.max_size {
            remaining = self.max_size;
        } else if remaining < center {
            center = remaining;
        }
        let mut index = start + self.min_size;
        center += start;
        ...
        let mut hash: u64 = 0;
        while index < center {
            hash = (hash << 1).wrapping_add(GEAR[self.source[index as usize] as usize]);
...
mod tests {
    ...
    fn test_cut_sekien_16k_chunks() {
        ...
        let expected: Vec<(u64, u64)> = vec![
            ...
            (6161241554519610597, 33362),
            (4509236223063678303, 30079),
            (2504464741100432583, 24700),
        ];
        for exp in expected.iter() {
            let (hash, pos) = chunker.cut(cursor, remaining);
            assert_eq!(hash, exp.0);
            assert_eq!(pos, cursor + exp.1);
            cursor += exp.1;
            remaining -= exp.1;
        }
    ...
    fn test_iter_sekien_16k_chunks() {
        ...
        assert_eq!(results[1].hash, 6161241554519610597);
        assert_eq!(results[1].offset, 21325);
        assert_eq!(results[1].length, 33362);
        assert_eq!(results[2].hash, 4509236223063678303);
        assert_eq!(results[2].offset, 54687);
        assert_eq!(results[2].length, 30079);
        assert_eq!(results[3].hash, 2504464741100432583);
        assert_eq!(results[3].offset, 84766);
        assert_eq!(results[3].length, 24700);
    }
}
